...
const menuItems = [
  ...
  { icon: Users, label: "Usuários", path: "/dashboard/users", adminOnly: true },
  { icon: Home, label: "Landing Page", path: "/", external: true, adminOnly: false },
];
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Acesse o painel para gerenciar os clientes capturados pela Landing Page do PARR.
            </p>
          </div>
            ...
            <div className="flex items-center gap-3 px-2 transition-all w-full">
              ...
              {!isCollapsed ? (
                <div className="flex items-center gap-2 min-w-0">
                  <img src={LOGO_FULL} alt="PrimeTax" className="h-7" />
                </div>
              ) : null}
            </div>
          </SidebarHeader>
          ...
          <SidebarContent className="gap-0">
            <SidebarMenu className="px-2 py-1">
              {menuItems.filter(item => !item.adminOnly || user?.role === "admin").map(item => {
                const isActive = !item.external && (item.path === "/dashboard" ? location === item.path : location.startsWith(item.path));
                return (
          ...
          <div className="flex border-b h-14 items-center justify-between bg-background/95 px-2 backdrop-blur supports-[backdrop-filter]:backdrop-blur sticky top-0 z-40">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                <img src={LOGO_SMALL} alt="PrimeTax" className="h-6" />
